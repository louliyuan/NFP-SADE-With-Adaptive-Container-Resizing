...
def normalize_vector(v):
    ...
    if almost_equal(length_squared, 1):
        return v
        ...
        while counter < 10 * (len_a + len_b):
            ...
            for i in range(0, len_a):
                ...
                for j in range(len_b):
                    ...
                    if almost_equal(
                        A['points'][i]['x'], B['points'][j]['x'] + B['offsetx']
                    ) and almost_equal(
            ...
            for i in range(0, len(touching)):
                ...
                if touching[i]['type'] == 0:
                    ...
                    vectors.append(
                        {
                            'x': vertex_a['A']['x']
                            - (vertex_b['A']['x'] + B['offsetx']),
                            'y': vertex_a['A']['y']
                            - (vertex_b['A']['y'] + B['offsety']),
                            'start': prev_a,
                            'end': vertex_a['A'],
                        }
                    )
            ...
            for i in range(0, len(vectors)):
                ...
                if (
                    prevvector
                    and (
                        vectors[i]['y'] * prevvector['y']
                        + vectors[i]['x'] * prevvector['x']
                    )
                    < 0
                ):
            ...
            if len(NFP) > 0:
                for i in range(0, len(NFP) - 1):
                    if almost_equal(
                        referencex,
                        NFP[i]['x'] and almost_equal(referencey, NFP[i]['y']),
                    ):
                        looped = True
...
def search_start_point(A, B, inside=True, NFP=None):
    """
    搜索 A 和 B 的排列，使得它们在给定 NFP 的情况下不重叠，仅搜索给定 NFP 中尚未遍历的起点
...
def inNfp(p, nfp):
    ...
    for i in range(0, len(nfp)):
        for j in range(0, len(nfp[i])):
            if almost_equal(p['x'], nfp[i][j]['x']) and almost_equal(
                p['y'], nfp[i][j]['y']
            ):
                return True
...
#     """
#     判断一个点是否位于多边形内部，基于射线法来确定一个点相对于闭合多边形的位置
#     """
...
def intersect(A, B):
    ...
    for i in range(0, len_a - 1):
        for j in range(0, len_b - 1):
            a1 = {
                'x': A['points'][i]['x'] + a_offsetx,
                'y': A['points'][i]['y'] + a_offsety,
            }
            a2 = {
                'x': A['points'][i + 1]['x'] + a_offsetx,
                'y': A['points'][i + 1]['y'] + a_offsety,
            }
            b1 = {
                'x': B['points'][j]['x'] + b_offsetx,
                'y': B['points'][j]['y'] + b_offsety,
            }
            b2 = {
                'x': B['points'][j + 1]['x'] + b_offsetx,
            ...
            if (
                ...
                and almost_equal(B['points'][pre_vb_index]['y'], B['points'][j]['y'])
            ):
                pre_vb_index = len_b - 1 if pre_vb_index == 0 else pre_vb_index - 1

            if (
                A['points'][pre_va_index] == A['points'][i]
                or almost_equal(A['points'][pre_va_index]['x'], A['points'][i]['x'])
                and almost_equal(A['points'][pre_va_index]['y'], A['points'][i]['y'])
            ):
            ...
            if (
                A['points'][next_a_index] == A['points'][i + 1]
                or almost_equal(A['points'][next_a_index]['x'], A['points'][i + 1]['x'])
                and almost_equal(
            ...
            a0 = {
                'x': A['points'][pre_va_index]['x'] + a_offsetx,
            ...
            a3 = {
                'x': A['points'][next_a_index]['x'] + a_offsetx,
                'y': A['points'][next_a_index]['y'] + a_offsety,
            }
            b3 = {
                'x': B['points'][next_b_index]['x'] + b_offsetx,
...
def line_intersect(A, B, E, F, infinite=None):
    """
    计算两条线段 AB 和 EF 的交点。它可以处理无限长直线的情况（通过 infinite 参数控制），也可以处理有限长线段的情况。
...
def polygon_projection_distance(A, B, direction):
    """
    计算将多边形 B 的点在某个特定方向上投影到多边形 A 上时的最小（或最负）投影距离。
    :param A:
    :param B:
    :param direction:
    :return:
    ...
    for i in range(0, len(edge_b)):
        ...
        for j in range(0, len(edge_a) - 1):
            ...
            if (
                abs(
                    (s2['y'] - s1['y']) * direction['x']
                    - (s2['x'] - s2['x']) * direction['y']
                )
                < TOL
            ):
                continue
...
def point_distance(p, s1, s2, normal, infinite=None):
    """
    计算点 p 到由 s1 到 s2 定义的直线段在某个特定法线方向 normal 上的投影距离。
    这个计算可以用来测量点到线段的垂直距离，并且可以处理无限长直线的情况。
    """
    ...
    if infinite is None:
        ...
        if (
            almost_equal(pdot, s1dot)
            and almost_equal(pdot, s2dot)
            and pdotnorm < s1dotnorm
            and pdotnorm < s2dotnorm
        ):
...
def polygon_area(polygon):
    """
    用鞋带公式计算多边形的面积，顶点逆时针排列则面积大于0，顶点按顺时针排列则面积小于0.
    """
    area = 0
    j = len(polygon) - 1
...
def rotate_polygon(polygon, angle):
    """
    将一个多边形绕原点旋转一个指定的角度，并计算旋转后多边形的新坐标和边界。
    """
    rotated = {'points': list()}
